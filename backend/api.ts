...
export class SpotifyAPI {
    ...
    public async organize(args: string, api: SpotifyAPI): Promise<Map<String, Number>> {
        const response = await api.scrapePlaylists('California');
        ...
        songlist.forEach((item: any) => {
            if (item.track) {
            songs.set(item.track.id, item.track.popularity);
            }
        });
        }

        const songOrder = new Map(Array.from(songs.entries()).sort((a, b) => Number(b[1]) - Number(a[1])));
        return songOrder;
    }
}
